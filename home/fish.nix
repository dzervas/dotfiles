{ pkgs, ... }: {
  ...
  programs.fish = {
    ...
    shellAliases = {
      update = ''pi-extension-bump && command grep --no-filename -r '# nix-update:' overlays/ | cut -d':' -f2 | xargs -L1 nix-update -f overlays/update-shim.nix && nix flake update --flake "$FLAKE_URL" --override-input nixpkgs github:NixOS/nixpkgs/$(curl -sL "https://monitoring.nixos.org/prometheus/api/v1/query?query=channel_revision" | jq -r ".data.result[] | select(.metric.channel==\"nixos-unstable\") | .metric.revision") && rebuild'';
