      ...
      in
      {
        ...
        custom =
          let
            ...
            jj-args = "--config signing.backend=none --color never --no-graph";
          in
          {
            ...
            jj_branch = {
              ...
              command = ''jj log ${jj-args} --ignore-working-copy -r 'closest_bookmark(@-)' --template 'bookmarks.join("/")' '';
            ...
            jj =
              let
                ...
                status-checks = lib.mapAttrsToList (
                  status: icon: ''if(${diff-status status}, "${icon}" ++ ${diff-status status}.len())''
                ) status-map;
                status-line = builtins.concatStringsSep ", " status-checks;
              in
              {
                command = ''
                  jj log ${jj-args} -n1 -r@  --template '
                              separate(" ",
                                ${status-line},
                                if(conflict, " "),
                                if(divergent, "⇕ "),
