/**
 * https://github.com/hjanuschka/shitty-extensions/blob/main/extensions/memory-mode.ts
...
class MemoryPreviewComponent {
	...
	render(width: number): string[] {
		...
		if (this.state === "loading") {
			...
			lines.push("");
			lines.push(
				truncateToWidth(this.theme.fg("accent", "  📄 ") + this.theme.bold(this.filePath), width),
			);
			lines.push(
				truncateToWidth(this.theme.fg("dim", "  " + "─".repeat(Math.min(60, contentWidth))), width),
			);
			lines.push("");
			...
			lines.push(
				truncateToWidth(
					this.theme.fg("dim", "  ←/→ to select, Enter to confirm, ESC to cancel"),
					width,
				),
			);
			lines.push("");
